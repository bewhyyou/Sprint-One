...
public class Tester {
    ...
    public void TestDog() throws Exception {
    	//use exact code to create. No matter what business object you're creating. No DogDAO, but BusinessObjectDAO
      Dog s = BusinessObjectDAO.getInstance().create("Dog", "dog11"); //"dog11" is the guid
      s.setBreed("Mutt");
      s.setDogName("Matty");
      Person emp = BusinessObjectDAO.getInstance().read("person1");
      s.setPerson(emp);
      s.save(); //since s is a business object, it calls the method in the business object class

      // since emp1 is in the Cache, this tests reading from the cache
      Dog s2 = BusinessObjectDAO.getInstance().read("dog11");
      assertSame(s, s2); //assert = JUnit. Assert that they're the same
      //same data

      // now clear the cache (you'd never do this in the real world)
      // then we can test reading from the database
      //assertEquals throws an exception (to the JUnit)
      Cache.getInstance().clear();
      ...
      //assertEquals: same pointer. Does it point to the same object
    ...
    public void TestPersonCar() throws Exception {
      ...
      Car car3 = BusinessObjectDAO.getInstance().read("car3");
